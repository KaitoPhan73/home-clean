...
export const columns: ColumnDef<TGroupResponse>[] = [
  ...
  //   accessorKey: "areaId",
  //   header: "ID Khu Vực",
  //   cell: ({ row }) => <div>{row.getValue("areaId")}</div>,
  // },
  // {
  //   accessorKey: "managerId",
  //   header: "ID Quản Lý",
  //   cell: ({ row }) => <div>{row.getValue("managerId")}</div>,
  // },
  // {
  //   accessorKey: "clusterIds",
  //   header: "Cụm Dịch Vụ",
  //   cell: ({ row }) => {
  //     const clusters = row.getValue<string[]>("clusterIds") || [];
  //     return <div>{clusters.length > 0 ? clusters.join(", ") : "Không có"}</div>;
  //   },
  // },
  // {
  ...
  //   header: "Dịch Vụ",
  //   cell: ({ row }) => {
  //     const services = row.getValue<string[]>("serviceIds") || [];
  //     return <div>{services.length > 0 ? services.join(", ") : "Không có"}</div>;
  //   },
  // },
  {
    id: "actions",
